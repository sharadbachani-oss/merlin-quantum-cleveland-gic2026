...
**Readout that survives noise (rubric §4.2 noise resilience).** Every failure on the way was measured, then engineered out: heavy-hex routing inflated 288 → 757 CX and killed the signal (0.016 recovery) → **swap network**, zero routing; bare amplitudes damp under the full light-cone → **protected ratio observables**; echo normalisers over- or under-correct → **calibrated mitigation stack** (Pauli twirling + TREX readout + multi-scale ZNE, IBM EstimatorV2 resilience 2) recovering 92% of amplitude; all-to-all closes at N ≈ 11 → **width scaling** on native topology, constant per-qubit depth, 156 qubits. Pair coherence is predicted from a measured device atlas before the first shot (median error 7–9% over 18 edges), so the survivable depth of a map is a number, not a trial.
...
**Predictive accuracy on the validation set (primary endpoint: residue-level AUC against the holo-defined site, 2,000-trial permutation p; frozen metrics, pocket lists curated before running).**
...
| **156-qubit K=6 map, C(28-7) at d = 11** | ibm_fez | **−0.0757 ± 0.0087 — 8.7σ vs zero, 2.4σ vs the map's own baseline; 99-qubit cone** |
...
![Figure 2 — 156-qubit K=6 connected-correlator front on KRAS (ibm_fez): max |C(i, active)| by lattice distance; the dashed line marks C(28-7) at distance 11, the map's most precisely measured cell.](C:/quantum ai 2026/CLEVELAND_SUBMISSION_PACKAGE/figures/fig1_k6_front.png)
...
**The crossing.** Real-time propagation on the derived force network with one residue per qubit: 156 qubits, six brickwork sweeps, connected-correlator front to lattice distance 22 — C(28-7) = −0.0757 ± 0.0087 at distance 11, 8.7σ from zero and 2.4σ above the map's own baseline, whose exact counterpart spans a 99-qubit connected cone; anchored by the K = 2 exact-light-cone pass at 156 qubits (−0.4584 ± 0.008 vs −0.4648) carried into the K = 6 regime with falsifiers built in; tensor-network adjudication of the K = 6 map — reproduce the device to a depth, fail past it, as already done on the 128-qubit spectrum — is PoC step 1, so the hardware claim is stated as a path. The biological validation is independent of the hardware claim: residue-level AUC 0.71–0.89 at p = 0.0005 on four of five Table-1 targets (myosin 0.71, BCR-ABL1 0.80, CDK2 0.87, glucokinase 0.89), above the strongest structure-only floor on two and level on a third, with the controller/receiver split landing where the mechanism says it should. **Where it is decisive:** past the ~40-qubit exact wall, on the K = 6 front whose cone spans 99 qubits — flown on KRAS, the cryptic-pocket case the dynamic map exists for; Phase 2 flies the remaining Table-1 maps and runs the tensor-network adversary on this observable as it was run on the 128-qubit lattice.
...
| R3 | 156q K=6 map, C(28-7) −0.0757 ± 0.0087 at d = 11 (8.7σ vs zero; 2.4σ vs the map's flat baseline; unreachable-pair spread sd 0.050 on the second anchor) | ibm_fez | `dab7i5mrrl7c7386gfh0` | `RESULT_residue_K6_20260901_131159.json`, prereg `13a08d7d` |
...
| 156q K=6 long-range correlator | measured (8.7σ vs zero, 2.4σ vs own baseline) | flat-baseline and exponential floors run on the map itself (`floor_cleveland_K6.json`); exact light-cone at K=2 (passed); TN adversary planned | hardware | 1 job / target, ≤ 60 min QPU | excess over the map's own baseline, not over zero |
| Ranking degrades when K=6 map is replaced by its classical approximation | planned (Phase 2, decisive) | single-excitation propagation, Ohm | hardware map vs classical | 4 jobs / month | paired AUC drop with CI |
